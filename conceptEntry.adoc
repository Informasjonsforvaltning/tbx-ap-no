...
`<transacGrp>` _here is recommended and should be used to specify date created or date last updated, which are recommended properties._
...
* basic:modification
** (PID http://datcatinfo.termweb.eu/datcat/DC-305) for dato sist oppdatert / _date last updated_
| Anvendelse / _Usage note_ |
...
`<descrip>` _here is optional and may be used to specify various concept relations which are all optional, when there is no need to describe the relations with additional information. Use `<descripGrp>` in `<langSec>` when there is need for additional information about a concept relation._
