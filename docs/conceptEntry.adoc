...
[cols="30s,70d"]
|===
| Navn / Name | ansvarlig virksomhet / responsible organization
| Datakategori / Data category | dct:creator
| PID | http://purl.org/dc/terms/creator
| TBX-nivå / TBX level |conceptEntry
| TBX-klassifisering / TBX Classification |admin
| Verdiområde / Range |xref
| Anvendelse / Usage note |Egenskapen brukes til å referere til virksomheten som er ansvarlig for beskrivelsen av begrepet, og som ikke er den samme som <<Begrep-publisert-av>>.
...
| Multiplisitet |0..n
| Kravnivå / Requirement level |Valgfri / Optional
| Merknad 1 / Note 1 |(norsk) Egenskapen/relasjonen kan også leses som «har spesifikt begrep».
...
| Multiplisitet |0..n
| Kravnivå / Requirement level |Valgfri / Optional
| Merknad 1 / Note 1 |(norsk) Egenskapen/relasjonen kan også leses som «har generisk begrep».
...
(English) Use <<Begrep-har-assosiativ-begrepsrelasjon>> when there is need to describe the relation role that the concept has.
|===
